...
var instrucciones = [
'Desliza las piezas por el espacio vacío para resolver el rompecabezas.',
'Las piezas pueden moverse de a una por vez, y solo en línea vertical y horizontal, nunca en diagonal.',
'Mueve las piezas por el recuadro hasta obtener la imágen que se muestra en la solución.'
];
...
function agregarUltimaDireccion(direccionDeFlecha){
...
function chequearSiGano() {
  ...
  for(var i= 0; i < grilla.length; i++){
    for(var j= 0; j < grilla[i].length; j++){
      nuevaGrilla.push(grilla[i][j] === grillaGanadora[i][j])
    }
  }//COMPLETAR
  function isTrue (parametro){
    return parametro === true;
  }
  if(nuevaGrilla.every(isTrue)){
...
function mostrarCartelGanador() {
  ...
  var modalCerrar = document.getElementById('botonX');
  var modalBoton = document.getElementById('botonModal');
...
function moverEnDireccion(direccion) {
  ...
  // Mueve pieza hacia la derecha, reemplazandola con la blanca
    ...
    nuevaColumnaPiezaVacia = columnaVacia - 1;//COMPLETAR
  }
